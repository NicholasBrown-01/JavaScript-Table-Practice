...
// *** GO LIVE WITH THIS AND WATCH ANY CHANGES YOU MAKE HAPPEN IN REAL TIME AS YOU MAKE THEM ***

// * For visualization, what you are creating here in JavaScript would look like the following if in HTML. *
...
function renderPracticeTable() {
  // 1. Assigning the practice-table and/or <table> tag we made in our index.html to the variable "practiceTable" for our JS to be able to render and use it.
  let practiceTable = document.getElementById('practice-table');

  // 2. Physically create a table and assign it to the variable "physicalTableBody" for our JS to create the actual table and then assign Child Elements such as 'thead', 'tbody', and 'tfoot'.
  let physicalTableBody = document.createElement('table');
  practiceTable.appendChild(physicalTableBody);

  // 3. Create a table/container to hold all the rows ('tr') and cells ('td') for our table. Just like if listed in an HTML document, here, the 'tbody' MUST also be a Child Element of a 'table'.
  let actualTableBody = document.createElement('tbody');
  physicalTableBody.appendChild(actualTableBody);


  
  let headerRow = document.createElement('tr'); // 4. Make a blank first row of the table for the Header Row
  ...
  headerCell1Row1.textContent = 'Header Cell 1, Row 1'; // 6. Set the text content of the 'headerCell1Row1'.
  ...
  let headerCell2Row1 = document.createElement('th'); // 8. Let's make the second cell that continues along our header row.
  headerCell2Row1.textContent = 'Header Cell 2, Row 1'; // 9. Set the text content of the 'headerCell2Row1'.
  headerRow.appendChild(headerCell2Row1); // 10. Append the 'headerCell2Row1' displaying the text we made, and
  actualTableBody.appendChild(headerRow); // 11. Append everything we just created within this 'headerRow' variable, and attach it to the 'actualTableBody' variable that created our 't-body' in Step 3.
  ...
  let row2 = document.createElement('tr'); // 1. Make a blank row.

  let row2cell1 = document.createElement('td'); // 2. Make a blank cell for data/text to go in.
 ...
 // TODO: Let's add a new "row 3":
  ...
  actualTableBody.appendChild(row3); // 5. Append everything we just created within this 'row3' variable to the 'actualTableBody' variable that created our 't-body' in Step 3
}
